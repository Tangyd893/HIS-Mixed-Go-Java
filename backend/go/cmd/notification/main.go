...
import (
	...
	"github.com/his-mixed/go/internal/notification/handler"
...
func main() {
	fmt.Println("=== HIS 消息通知服务启动 ===")

	// 加载配置
	cfg, err := config.Load("configs/notification.yaml")
	if err != nil {
...
func startHTTPServer(port int) *http.Server {
	...
	go func() {
		log.Printf("通知服务 HTTP 启动在端口 %d", port)
...
func startGRPCServer(port int, h *handler.NotificationHandler) (*grpc.Server, net.Listener) {
	...
	go func() {
		log.Printf("通知服务 gRPC 启动在端口 %d", port)
